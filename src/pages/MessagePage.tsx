...
export function MessagePage() {
  ...
  const channels: ChatChannel[] = [
    {
      id: 'chat_alpha_direct',
      name: '株式会社アルファ通信',
      title: '週末キャンペーンスタッフ2名',
      status: 'negotiating',
      avatar: 'A',
      avatarBg: 'bg-blue',
      preview: '明日の待ち合わせ時間は10時でお願いします。',
      time: '10:42'
    },
    {
      id: 'chat_beta_direct',
      ...
      avatar: 'B',
      avatarBg: 'bg-green',
      preview: '発注書を発行しました。ご確認お願いします。',
      time: '昨日'
    },
    {
      id: 'chat_gamma_direct',
      name: 'ガンマモバイル',
      title: 'ドコモショップ応援（3日間）',
      status: 'contracted',
      avatar: 'G',
  ...
  const getDefaultMessages = (chatId: string) => {
    ...
    if (chatId === 'chat_alpha_direct') {
      return [
        ...
        { type: 'received', senderName: '株式会社アルファ通信', text: 'お世話になっております。週末のキャンペーンスタッフ2名の件ですが、まだ募集されていますでしょうか？', time: '10:30', avatar: 'A' },
        { type: 'sent', text: 'お世話になっております！はい、まだ募集しております。\n単価15,000円でお願いしたいのですが、いかがでしょうか？', time: '10:35' },
        { type: 'received', senderName: '株式会社アルファ通信', text: '明日の待ち合わせ時間は10時でお願いします。', time: '10:42', avatar: 'A' }
      ];
    }
    if (chatId === 'chat_beta_direct') {
      return [
        { type: 'system', text: 'チャットを開始しました', time: '' },
        { type: 'received', senderName: 'ベータエージェンシー', text: '発注書を発行しました。ご確認お願いします。', time: '昨日', avatar: 'B' }
      ];
    }
    return [
      { type: 'system', text: 'チャットを開始しました', time: '' },
      { type: 'received', senderName: 'ガンマモバイル', text: 'よろしくお願いします。', time: '月曜日', avatar: 'G' }
    ];
  };
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('chat_messages_' + activeChat.id);
    if (stored) {
      setMessages(JSON.parse(stored));
    } else {
      const def = getDefaultMessages(activeChat.id);
      setMessages(def);
      localStorage.setItem('chat_messages_' + activeChat.id, JSON.stringify(def));
    }
  }, [activeChat]);
  ...
  const handlePropose = () => {
    setProposed(true);
    const newMsg1 = { type: 'system', text: 'あなたが電子発注書を発行しました', time: '' };
    const newMsg2 = { type: 'sent', text: '[CONTRACT_MOCK]', time: 'たった今' };
    ...
    setTimeout(() => {
      const systemConfirm = { type: 'system', text: '相手が条件を承認し、Stripe決済（仮押さえ）が完了しました', time: '' };
      const replyMsg = { type: 'received', senderName: activeChat?.name || '相手', text: '発注ありがとうございます！承認いたしました。当日はよろしくお願いいたします。', time: 'たった今', avatar: activeChat?.avatar || 'A' };
      
      setMessages(prev => {
        const next = [...prev, systemConfirm, replyMsg];
        if (activeChat) {
          localStorage.setItem('chat_messages_' + activeChat.id, JSON.stringify(next));
        }
        return next;
      });
    }, 3000);
      ...
      <main className="list-area">
        <div className="chat-list">
          {channels.map((channel) => (
            <div key={channel.id} className="chat-item" onClick={() => setActiveChat(channel)}>
